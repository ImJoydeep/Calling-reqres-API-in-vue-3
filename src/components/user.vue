<template>
    ...
    <div class="container text-lg font-bold">
        <table class="table-auto" id="table">
            ...
            <tbody>
                <tr>
                    <RouterLink :to="{name: 'multiUserView', params: {first_name:user.first_name, last_name: user.last_name, id:user.id}}">
                        <td class="hover:underline">{{ user.first_name }} {{ user.last_name }}</td>
                    </RouterLink>
                </tr>
            </tbody>
        </div>
